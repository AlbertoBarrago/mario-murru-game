...
export default class Enemy {
  ...
  update(canvasWidth) {
    ...
    if (this.platformBounds) {
      ...
      if (
        this.x <= this.platformBounds.startX ||
        this.x + this.width >= this.platformBounds.endX
      ) {
        this.direction *= -1;
        // Clamp position to prevent enemy from going off platform
        this.x = Math.max(
          this.platformBounds.startX,
          Math.min(this.x, this.platformBounds.endX - this.width),
        );
      }
    } else {
  ...
  render(ctx) {
    ...
    if (enemySprite) {
      ...
      ctx.drawImage(
        ...
        sourceY,
        32,
        32, // Source coordinates from sprite sheet
        this.x,
        this.y,
        this.width,
        this.height, // Destination coordinates on canvas
      );
    } else {
      // Fallback rendering
