...
export function togglePause() {
  ...
  if (gameState.paused) {
    ...
    if (!sounds.backgroundMusic.muted) {
...
export function checkCollisions() {
  ...
  // Enemy collisions
  for (let index = gameState.enemies.length - 1; index >= 0; index--) {
...
/**
 * Check if a new platform overlaps with existing platforms
 * @param {Object} newPlatform - The platform to check
 * @param {Array} existingPlatforms - Array of existing platforms
 * @param {number} minSpacing - Minimum spacing between platforms
 * @returns {boolean} - True if there's an overlap
 */
function checkPlatformOverlap(newPlatform, existingPlatforms, minSpacing = 30) {
...
export function loadNextLevel() {
  ...
  // Add additional platforms with overlap prevention
  for (let i = 0; i < platformCount; i++) {
    ...
    while (!validPlatform && attempts < maxAttempts) {
      ...
      if (!checkPlatformOverlap(newPlatform, gameState.platforms)) {
        validPlatform = true;
      }
    }

    // Only add platform if we found a valid position
